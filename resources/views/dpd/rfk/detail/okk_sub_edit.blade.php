            ...
            <div class="card-body table-responsive">
                ...
                <table class="table table-sm" style="border: 1px solid black; text-align:center">
                    <thead>
                        ...
                        @if ($subitem->id == $okksub_id)
                        <tr style="font-size:12px; background-color:rgb(211, 247, 236)">

                            <form method="post"
                                action="/dpd/rfk/detail/{{$id}}/okk/sub/{{$item->id}}/edit/{{$subitem->id}}">
                                @csrf
                                <td>
                                    <button type="submit" class="btn btn-xs btn-primary">UPDATE</button>
                                    <br /><br />
                                    <a href="/dpd/rfk/detail/{{$id}}/okk"
                                        class="btn btn-danger btn-xs btn-block">BATAL</a>
                                </td>
                                <td style="border: 1px solid black"></td>
                                <td style="border: 1px solid black" class="text-left">
                                    <textarea rows="5" cols="30" name="kode_akun">{{$subitem->kode_akun}}</textarea>
                                </td>
                                <td style="border: 1px solid black" class="text-left">
                                    <textarea cols="50" rows="5" name="program">{{$subitem->program}}</textarea>
                                </td>
                                <td style="border: 1px solid black" class="text-left">
